/*
* Controller to VirtualPad example.
* Reads a source controller and translates to VirtualPad.
...
#endif
JoystickShield::JoystickShieldVirtualPad<JoyXPin, JoyYPin, 4, 3, 5, 2, 8, 6, 7> Pad{};
#define DEBUG_ANALOG_PAD
...
Nintendo64Controller::Nintendo64VirtualPad<> Pad{};
bool Polling = false;
#define DEBUG_ANALOG_PAD
...
GameCubeController::GameCubeVirtualPad<> Pad{};
#define DEBUG_ANALOG_PAD
#elif defined(USE_ATARI_CONTROLLER)
enum class AtariPin : uint8_t { Up = 2, Down = 3, Left = 4, Right = 5, Button = 6 };
AtariJoystick::AtariJoystickVirtualPad<AtariPin> Pad{};
...
MegaDriveController::MegaDriveControllerVirtualPad<MegaDrivePin> Pad{};
#else
#define DEBUG_MOTION_PAD
#endif
...
void setup()
{
	...
	while (!Serial)
		;
	delay(1000);
	Serial.print(F("Controller to VirtualPad..."));
	...
	JoyBusReader.Start();
#elif defined(USE_ATARI_CONTROLLER) || defined(USE_MEGA_DRIVE_CONTROLLER)
	Pad.Setup();
#endif
...
void LogActions()
{
	...
	if (Actions.DPad() != VirtualPad::DPadEnum::None)
	{
		Serial.print(F("DPad::"));
	...
	if (Actions.Joy1() != VirtualPad::DPadEnum::None)
	{
		Serial.print(F("Joy1::"));
	...
	if (Actions.Joy2() != VirtualPad::DPadEnum::None)
	{
		Serial.print(F("Joy2::"));
...
void PrintDPadActions(const VirtualPad::DPadEnum dpad)
{
	switch (dpad)
	{
	case VirtualPad::DPadEnum::Up:
